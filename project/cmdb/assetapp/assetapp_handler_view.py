...
-------------------------------------------------------------
    Creator : 汪春旺
       Date : 2018-03-27
    Project : cmdb
   FileName : assetapp_handler_view.py
Description : 
-------------------------------------------------------------
"""
import json

from assetapp.models import NewAssetApprovalZone
...
def log(log_type, request=None, msg=None, asset=None, new_asset=None):
    """
    记录日志
    """
...
class ApproveAsset(object):
    ...
    def __init__(self, request, asset_id):
        self.request = request
        self.new_asset = NewAssetApprovalZone.objects.get(id=asset_id)
    ...
    def _create_manufacturer(self, asset):
        ...
        manuf_name = self.new_asset.manufacturer
        if manuf_name:
            manuf_obj, boo = models.Manufacturer.objects.get_or_create(name=manuf_name)
            asset.manufacturer = manuf_obj
            asset.save()
    ...
    def _create_CPU(self, asset):
        """创建CPU"""
        models.CPU.objects.create(asset=asset, cpu_model=self.new_asset.cpu_model, cpu_count=self.new_asset.cpu_count)
...
class UpdateAsset(object):
    ...
    def __init__(self, request, asset_obj, report_data):
        self.request = request
    ...
    def _server_update(self):
        try:
            ...
            log('update_failed', msg=e, asset=self.asset_obj, request=self.request)
            print(e)
        else:
            # 添加日志
            log("update", asset=self.asset_obj)
            print("资产数据被更新!")
    ...
    def _update_manufacturer(self):
        ...
        if m:
            ...
            self.asset_obj.manufacturer = manufacturer_obj
        else:
            self.asset_obj.manufacturer = None
        self.asset_obj.manufacturer.save()

    def _update_server(self):
        """
        更新服务器
        """
        self.asset_obj.server.model = self.report_data.get('model')
        self.asset_obj.server.os_type = self.report_data.get('os_type')
        self.asset_obj.server.os_distribution = self.report_data.get('os_distribution')
        self.asset_obj.server.os_release = self.report_data.get('os_release')
        self.asset_obj.server.save()
    ...
    def _update_CPU(self):
        ...
        :return:
        """
        self.asset_obj.cpu.cpu_model = self.report_data.get('cpu_model')
        self.asset_obj.cpu.cpu_count = self.report_data.get('cpu_count')
        self.asset_obj.cpu.cpu_core_count = self.report_data.get('cpu_core_count')
        self.asset_obj.cpu.save()
    ...
    def _update_RAM(self):
        ...
        """
        # 获取已有内存信息，并转成字典格式
        old_rams = models.RAM.objects.filter(asset=self.asset_obj)
        old_rams_dict = dict()
        if old_rams:
            for ram in old_rams:
        ...
        if new_rams_dict:
            for key in new_rams_dict:
                ...
                models.RAM.objects.update_or_create(asset=self.asset_obj, slot=key, defaults=defaults)

    def _update_disk(self):
        """
        更新硬盘信息。类似更新内存。
        """
        old_disks = models.Disk.objects.filter(asset=self.asset_obj)
        old_disks_dict = dict()
        if old_disks:
        ...
        if new_disks_dict:
            for key in new_disks_dict:
                ...
                models.Disk.objects.update_or_create(asset=self.asset_obj, sn=key, defaults=defaults)

    def _update_nic(self):
        """
        更新网卡信息。类似更新内存。
        """
        old_nics = models.NIC.objects.filter(asset=self.asset_obj)
        old_nics_dict = dict()
        if old_nics:
            for nic in old_nics:
        ...
        if new_nics_dict:
            for key in new_nics_dict:
                ...
                models.NIC.objects.update_or_create(asset=self.asset_obj, model=new_nics_dict[key]['model'],
